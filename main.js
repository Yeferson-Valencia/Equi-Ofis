  ...
  .then(data => {
    ...
    categoryLinks.forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            const productosFiltrados = productos.filter(producto => producto.Clase === categoria);
            console.log(productosFiltrados);

            // Mostrar solo los productos que cumplen con la condición
...
function mostrarProductos(productos, paginaActual) {
    ...
    prevPageButton.addEventListener('click', () => {
        if (paginaActual > 1) {
            mostrarProductos(productos, paginaActual - 1);
        }
    });
...
prevPageButton.addEventListener('click', () => {
    if (paginaActual > 1) {
        mostrarProductos(productos, paginaActual - 1);
        nextPageButton.removeAttribute('disabled'); // Habilita el botón de siguiente página si estaba deshabilitado
    }
});
...
nextPageButton.addEventListener('click', () => {
    if (paginaActual < numPaginas) {
        ...
        if (paginaActual + 1 === numPaginas) {
            nextPageButton.setAttribute('disabled', 'disabled'); // Deshabilita el botón de siguiente página si es la última página
        }
    }
});
...
window.onload = function() {
    ...
    const productos = Array.from({ length: 50 }, (_, i) => ({ Nombre: `Producto ${i + 1}`, Ruta: 'imagen.jpg' }));

    // Muestra los productos en la página 1
